...
- (id)initWithAppKey:(NSString *)appKey appSecret:(NSString *)appSecret text:(NSString *)text image:(UIImage *)image
{
    ...
    if (self = [super initWithFrame:bounds])
    {
        ...
        [panelImageView setImage:[[UIImage imageNamed:@"Sinabg.png"] stretchableImageWithLeftCapWidth:18 topCapHeight:18]];
		...
		[closeButton setTitle:NSLocalizedStringFromTable(@"关闭", @"SinaLocalizable", nil) forState:UIControlStateNormal];
        ...
        [titleLabel setText:NSLocalizedStringFromTable(@"新浪微博", @"SinaLocalizable", nil)];
		...
		[sendButton setTitle: NSLocalizedStringFromTable(@"发送", @"SinaLocalizable", nil) forState:UIControlStateNormal];
...
- (void)onSendButtonTouched:(id)sender
{
    ...
    {
		UIAlertView * alertView = [[UIAlertView alloc] initWithTitle:NSLocalizedStringFromTable(@"新浪微博", @"SinaLocalizable", nil)
                                                             message:NSLocalizedStringFromTable(@"请输入微博内容", @"SinaLocalizable", nil)
                                                            delegate:nil
                                                   cancelButtonTitle:NSLocalizedStringFromTable(@"确定", @"SinaLocalizable", nil) otherButtonTitles:nil];
		[alertView show];
		[alertView release];
		return;
	}
...
- (void)sizeToFitOrientation:(UIInterfaceOrientation)orientation
{
    ...
    if (UIInterfaceOrientationIsLandscape(orientation))
    {
        ...
        [baseView setFrame:CGRectMake((bounds.size.width - 480) / 2, (bounds.size.height - 320) / 2, 480, 320)];
    ...
    {
        [self setFrame:bounds];
        [baseView setFrame:CGRectMake((bounds.size.width - 320) / 2, (bounds.size.height - 480) / 2, 320, 480)];
        ...
        [panelImageView setImage:[UIImage imageNamed:@"Sinabg.png"]];
